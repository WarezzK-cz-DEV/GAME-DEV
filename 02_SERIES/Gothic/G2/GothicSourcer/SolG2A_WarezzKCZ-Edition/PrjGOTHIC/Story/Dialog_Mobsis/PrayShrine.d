...
var int no_pays_done;
var int small_pays_done;
var int medium_pays_done;
var int big_pays_done;
const int MAX_NO_PAYS = 100;
const int MAX_SMALL_PAYS = 60;
const int MAX_MEDIUM_PAYS = 30;
const int MAX_BIG_PAYS = 15;
...
{
	if((NO_PAYS_DONE == 0) && (SMALL_PAYS_DONE == 0) && (MEDIUM_PAYS_DONE == 0) && (BIG_PAYS_DONE == 0))
	{
		NO_PAYS_DONE = 0;
		SMALL_PAYS_DONE = 0;
		MEDIUM_PAYS_DONE = 0;
	...
	else
	{
		...
		if((Npc_HasItems(hero,itmi_gold) >= 50) && (SMALL_PAYS_DONE < MAX_SMALL_PAYS))
		{
			Info_AddChoice(pc_prayshrine_pray,"I want to pray and donate 50 gold pieces.",pc_prayshrine_pray_smallpay);
		};
		if((Npc_HasItems(hero,itmi_gold) >= 100) && (MEDIUM_PAYS_DONE < MAX_MEDIUM_PAYS))
		{
			Info_AddChoice(pc_prayshrine_pray,"I want to pray and donate 100 gold pieces.",pc_prayshrine_pray_mediumpay);
		};
		if((Npc_HasItems(hero,itmi_gold) >= 200) && (BIG_PAYS_DONE < MAX_BIG_PAYS))
		{
			Info_AddChoice(pc_prayshrine_pray,"I want to pray and donate 200 gold pieces.",pc_prayshrine_pray_bigpay);
		};
	};
};
...
{
	var int percent;
	percent = Hlp_Random(100);
	if((PRAYDAY == Wld_GetDay()) || (NO_PAYS_DONE > MAX_NO_PAYS))
	{
...
func void pc_prayshrine_pray_smallpay()
{
	...
	percent = Hlp_Random(100);
	if((percent < 50) || (SMALL_PAYS_DONE > MAX_SMALL_PAYS))
	{
		PrintScreen(PRINT_BLESSNONE,-1,-1,FONT_SCREEN,2);
	}
	else
	...
	SMALL_PAYS_DONE = SMALL_PAYS_DONE + 1;
	Info_ClearChoices(pc_prayshrine_pray);
};
...
func void pc_prayshrine_pray_mediumpay()
{
	...
	percent = Hlp_Random(100);
	if((percent > 80) || (MEDIUM_PAYS_DONE > MAX_MEDIUM_PAYS))
	{
		PrintScreen(PRINT_BLESSNONE,-1,-1,FONT_SCREEN,2);
	}
	else
	...
	MEDIUM_PAYS_DONE = MEDIUM_PAYS_DONE + 1;
	Info_ClearChoices(pc_prayshrine_pray);
};
...
{
	var int percent;
	percent = Hlp_Random(3);
	Npc_RemoveInvItems(hero,itmi_gold,200);
	if((hero.guild != GIL_KDF) && (hero.guild != GIL_NOV) && (percent == 0))
	{
		b_blessattribute(hero,ATR_STRENGTH,1);
	}
	else if((hero.guild != GIL_KDF) && (hero.guild != GIL_NOV) && (percent == 1))
	{
		b_blessattribute(hero,ATR_DEXTERITY,1);
	}
	else if((hero.guild != GIL_SLD) && (hero.guild != GIL_DJG) && (percent != 2))
	{
